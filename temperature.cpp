...
void manage_heater()
{
  float pid_input;
  float pid_output;
  static unsigned char temp_flg = 0x00;
  ...
  for(int e = 0; e < EXTRUDERS; e++) 
  {
	...
	if(watchmillis[e] && millis() - watchmillis[e] > WATCH_TEMP_PERIOD)
	{
		...
		{
			if(degHotend(e) < EXTRUDE_MINTEMP)
			{
				kill_heater_err();
			}
			else
			{
				if(degTargetHotend(e)>(degHotend(e)+10))
				{
					kill_heater_err();
				}
			}
		}else{
			if(degTargetHotend(e)>80)  //heater in heating mode
			{				
				setWatch();
			}
			else
	...
	if((temp_flg != 0x03)||(degTargetHotend(e)==0))  
	{
		temp_flg = 0x00;
	}
	  if(degTargetHotend(e)>80)  //heater in heating mode
