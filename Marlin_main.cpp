...
void setup()
{
  ...
  // filament monitor mod - Ziggy 18/03/2015
// filament monitor pin set to input
SET_INPUT(11);
WRITE(11,HIGH); // enable pull up
}
// filament monitor mod - Ziggy 18/03/2015
...
//
if (digitalRead(11) == HIGH) { // check filament monitor alarm pin
...
#ifdef LAVA
void kill_heater_err()
{ 
  lcd_heater_err_display();
  _delay_ms(1000);	// wait 1sec to display the splash screen
  cli(); // Stop interrupts	
  disable_heater();
  disable_x();
